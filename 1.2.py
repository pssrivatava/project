...
#path_to_image=cv2.imread('G:\circles.jpg')
image1=cv2.imread('G:pics\Image1.jpg')
def aruco_detect():
    ...
    lower_blue2 = np.array([120,255,255])
    upper_blue1 = np.array([221,100,100])
    upper_blue2 = np.array([240,255,255])
    
    mask1 = cv2.inRange(hsv, lower_red1, lower_red2)
    mask2 = cv2.inRange(hsv, upper_red1, upper_red2)

    mask3 = cv2.inRange(hsv, lower_blue1, lower_blue2)
    mask4 = cv2.inRange(hsv, upper_blue1, upper_blue2)
    ...
    dest2=cv2.add(res3,res4)
    dest3=cv2.add(dest1,dest2)
    #cv2.imshow("res",dest3)

    d1=cv2.add(mask1,mask2)
    d2=cv2.add(mask3,mask4)
    d3=cv2.add(d1,d2)
    cv2.imshow("mask",d3)
    ...
    edges = cv2.Canny(d3,150,250,apertureSize=3)
